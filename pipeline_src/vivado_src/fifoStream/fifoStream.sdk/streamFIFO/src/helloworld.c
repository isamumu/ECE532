...
//		*(SourceAddr + i) = (i+1) << 7;
	...
	for(int j = 0; j < 67; j++){
		...
		while (1) {
			...
			// xil_printf("recv %d ", ReceivedCount);
			// m += 1;
		...
		if(n == 65){
			xil_printf("--> ");
			for(int i = 0; i < 64; i++){
				*(SourceAddr + i) = BUFFER[i] << 7;
			}
			// xil_printf("{packet #%d} ", BUFFER[64]);
			...
			xil_printf("successfully send packet #%d", BUFFER[64], j);
