...
def get_file_path():
    global file_path
    # Open and return file path
    file_path= filedialog.askopenfilename(title = "Select A File", filetypes = (("mov files", "*.jpeg"), ("mp4", "*.mp4"), ("wmv", "*.wmv"), ("avi", "*.avi")))
    l1 = Label(window, text = "File path: " + file_path).pack()

window = Tk()
window.title("Image Compressor")
window.geometry("200x200")
# logo = tk.PhotoImage(file="cat.jpeg")

# w1 = tk.Label(window, image=logo).pack(side="right")

explanation = """Welcome to our Image Compressor. 
To compress an image, please select a file 
by pressing the above button"""
logo = tk.PhotoImage(file="xilinx.png")
# w1 = tk.Label(window, image=logo).pack(side="right")
w2 = tk.Label(window, 
              compound=tk.CENTER,
              padx = 1, 
...
b1 = Button(window, text = "Please Select File", width = 50, command = get_file_path).pack()
window.mainloop()
print(file_path)
