...
{
    int m;
    for (m = 0; m < PICTURE_SIZE/2; m++)
    {
        am_values[m] = 0.5 * cos((2*m + 1)*l*PI/(2*PICTURE_SIZE));
        if (l == 0)
        {
...
void compute_um (int l, int* xm_values, int* um_values)
{
    ...
    for (m = 0; m < PICTURE_SIZE/2; m++)
    {
        switch (l%2)
        {
            case 0:
                // l is even
                um_values[m] = xm_values[m] + xm_values[PICTURE_SIZE-1-m];
                break;
            case 1:
                // l is odd
                um_values[m] = xm_values[m] - xm_values[PICTURE_SIZE-1-m];
...
float compute_dct_coeff(int l, int* xm_values)
{
    ...
    float* am_values = (float*) malloc( PICTURE_SIZE / 2 * sizeof(float));
    compute_am(l, am_values);
    // for (int i = 0; i < PICTURE_SIZE / 2; i++)
    // {
    //     printf("a%d: %.2f\n", i, am_values[i]);
    // }
    // 2. Calculate um values (sums and differences of xm terms)
    signed int * um_values = (int*) malloc( PICTURE_SIZE / 2 * sizeof(signed int));
...
{
    // Input: matrix with PICTURE_SIZE x PICTURE_SIZE values
    // Output: matrix with PICTURE_SIZE x PICTURE_SIZE coefficients
    // i represents the rows, j represents the columns
    int i;
    int j;
    // 1. Perform 1-D DCT on the rows of the image
    int* xm_values = (int*) malloc(PICTURE_SIZE * sizeof(int));
    float* yl_values = (float*) malloc(PICTURE_SIZE * sizeof(float));
...
void compute_inverse_al (int l, float* al_values, int is_odd)
{
    ...
    for (m = 0; m < PICTURE_SIZE/2; m++)
    {
        ...
        al_values[m] = 0.5 * cos((2*l + 1)*n*PI/(2*PICTURE_SIZE));
        if (n == 0)
        {
...
struct Tuple compute_inverse_dct_coeff(int l, float* ym_values)
{
    // printf("l: %d\n", l);
    // 1. Calculate am values (cosine values)
    float* al_values_even = (float*) malloc( PICTURE_SIZE / 2 * sizeof(float));
    compute_inverse_al(l, al_values_even, 0);
    // for (int i = 0; i < PICTURE_SIZE/2; i++)
    // {
    //     printf("Even AL Value %d: %.2f\n", i, al_values_even[i]);
    // }
    float* al_values_odd = (float*) malloc( PICTURE_SIZE / 2 * sizeof(float));
...
void compute_one_dimensional_inverse_dct(float* yl_values, float* xm_values)
{
    ...
    for (l = 0; l < PICTURE_SIZE / 2; l++)
    {
        ...
        xm_values[PICTURE_SIZE-1-l] = pixels.x_l_other_end;
    }
}

void compute_inverse_dct(float* dct_coeffs, float* image)
{
    // Input: matrix with PICTURE_SIZE x PICTURE_SIZE coefficients
    // Output: matrix with PICTURE_SIZE x PICTURE_SIZE pixels
    // i represents the rows, j represents the columns
    int i;
    int j;
    // 1. Perform 1-D IDCT on the rows of the coefficients
    float* xm_values = (float*) malloc(PICTURE_SIZE * sizeof(float));
    float* yl_values = (float*) malloc(PICTURE_SIZE * sizeof(float));
    for (i = 0; i < PICTURE_SIZE; i++)
    {
        for (j = 0; j < PICTURE_SIZE; j++)
